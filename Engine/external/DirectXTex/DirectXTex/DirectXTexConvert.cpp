...
namespace
{
    ...
    inline float FloatFrom7e3(uint32_t Value)
    {
        ...
        uint32_t Result = ((Exponent + 124) << 23) | 
            (Mantissa << 16);          
    ...
    inline float FloatFrom6e4(uint32_t Value)
    {
        ...
        uint32_t Result = ((Exponent + 120) << 23) | 
            (Mantissa << 17);          
    ...
    DXGI_FORMAT format)
{
    ...
    case DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS:
    {
        ...
        if (size >= psize)
        {
            ...
            {
                if (dPtr >= ePtr) break;
                *(dPtr++) = XMVectorSet(sPtr[0], 0.f , 0.f, 1.f);
                sPtr += 2;
            }
            return true;
        }
    ...
    case DXGI_FORMAT_X32_TYPELESS_G8X24_UINT:
    {
        ...
        if (size >= psize)
        {
            ...
            for (size_t icount = 0; icount < (size - psize + 1); icount += psize)
            {
                ...
                if (dPtr >= ePtr) break;
                *(dPtr++) = XMVectorSet(0.f , static_cast<float>(*pg8), 0.f, 1.f);
                sPtr += 2;
            }
            return true;
        }
    ...
    case DXGI_FORMAT_R24_UNORM_X8_TYPELESS:
        if (size >= sizeof(uint32_t))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(uint32_t) + 1); icount += sizeof(uint32_t))
            {
                ...
                ++sPtr;
                if (dPtr >= ePtr) break;
                *(dPtr++) = XMVectorSet(r, 0.f , 0.f, 1.f);
            }
            return true;
        }
        return false;
    ...
    case DXGI_FORMAT_X24_TYPELESS_G8_UINT:
        if (size >= sizeof(uint32_t))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(uint32_t) + 1); icount += sizeof(uint32_t))
            {
                ...
                ++sPtr;
                if (dPtr >= ePtr) break;
                *(dPtr++) = XMVectorSet(0.f , g, 0.f, 1.f);
            }
            return true;
        }
        return false;
