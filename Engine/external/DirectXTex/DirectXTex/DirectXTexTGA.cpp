...
namespace
{
    ...
    enum TGADescriptorFlags
    {
        ...
        TGA_FLAGS_INTERLEAVED_2WAY = 0x40, 
        TGA_FLAGS_INTERLEAVED_4WAY = 0x80, 
    };
...
#pragma pack(pop)
    ...
    enum CONVERSION_FLAGS
    {
        ...
        CONV_FLAGS_INVERTX = 0x2,      
        CONV_FLAGS_INVERTY = 0x4,      
        CONV_FLAGS_RLE = 0x8,      

        CONV_FLAGS_SWIZZLE = 0x10000,  
        CONV_FLAGS_888 = 0x20000,  
    };
        ...
        _In_ size_t inSize)
    {
        ...
        if (inSize >= 4 && outSize >= 3)
        {
            ...
            for (size_t count = 0; count < (inSize - 3); count += 4)
            {
                ...
                *(dPtr++) = uint8_t(t & 0xFF);              
                *(dPtr++) = uint8_t((t & 0xFF00) >> 8);     
                *(dPtr++) = uint8_t((t & 0xFF0000) >> 16);  
            }
        }
    }
}
