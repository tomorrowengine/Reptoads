        ...
        namespace {\
            ...
            struct TestName{\
                ...
                TestName(Ts...names){\
                    ...
                    using expander = int[];\
                    (void)expander{(Catch::AutoReg( Catch::makeTestInvoker( &TestFunc<Types> ), CATCH_INTERNAL_LINEINFO, Catch::StringRef(), Catch::NameAndTags{ names, Tags } ), 0)... }; \
                }\
            };\
        ...
        namespace {                                                   \
            ...
            struct TestName {                                         \
                TestName() {                                          \
                    ...
                    int index = 0;                                    \
                    using expander = int[];                           \
                    (void)expander{(Catch::AutoReg( Catch::makeTestInvoker( &TestFuncName<Types> ), CATCH_INTERNAL_LINEINFO, Catch::StringRef(), Catch::NameAndTags{ Name " - " + Catch::StringMaker<int>::convert(index++), Tags } ), 0)... }; \
                }                                                     \
            };                                                        \
            static int INTERNAL_CATCH_UNIQUE_NAME( globalRegistrar ) = [](){ \
        ...
        namespace{ \
            ...
            struct TestNameClass{\
                ...
                TestNameClass(Ts...names){\
                    CATCH_INTERNAL_CHECK_UNIQUE_TYPES(CATCH_REC_LIST(INTERNAL_CATCH_REMOVE_PARENS, __VA_ARGS__)) \
                    using expander = int[];\
                    (void)expander{(Catch::AutoReg( Catch::makeTestInvoker( &TestName<Types>::test ), CATCH_INTERNAL_LINEINFO, #ClassName, Catch::NameAndTags{ names, Tags } ), 0)... }; \
                }\
            };\
        ...
        namespace {\
            ...
            struct TestNameClass{\
                TestNameClass(){\
                    ...
                    int index = 0;\
                    using expander = int[];\
                    (void)expander{(Catch::AutoReg( Catch::makeTestInvoker( &TestName<Types>::test ), CATCH_INTERNAL_LINEINFO, #ClassName, Catch::NameAndTags{ Name " - " + Catch::StringMaker<int>::convert(index++), Tags } ), 0)... }; \
                }\
            };\
            static int INTERNAL_CATCH_UNIQUE_NAME( globalRegistrar ) = [](){\
                using TestInit = combine<INTERNAL_CATCH_REMOVE_PARENS(TmplTypes)>\
...
namespace {
    ...
    bool useColourOnPlatform() {
        return
            ...
            !isDebuggerActive() &&
#endif
#if (!(defined(__DJGPP__) && defined(__STRICT_ANSI__)) && !defined(PLATFORM_PS)) //TODO: MARKED, Added check for PLATFORM_WINDOWS
