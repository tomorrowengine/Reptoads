...
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        ...
        if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
        {
            ...
            if (item_type == 2) { ret = ImGui::Checkbox("ITEM: CheckBox", &b); }                            
            if (item_type == 3) { ret = ImGui::SliderFloat("ITEM: SliderFloat", &col4f[0], 0.0f, 1.0f); }   
            if (item_type == 4) { ret = ImGui::ColorEdit4("ITEM: ColorEdit4", col4f); }                     
                    ...
                    for (int base = 0; base < 0x10000; base += 256)
                    {
                        ...
                        if (count > 0 && ImGui::TreeNode((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base+255, count, count > 1 ? "glyphs" : "glyph"))
                        {
                            ...
                            for (int n = 0; n < 256; n++)
                            {
                                ...
                                if (glyph)
                                    font->RenderChar(draw_list, cell_size, cell_p1, ImGui::GetColorU32(ImGuiCol_Text), (ImWchar)(base+n)); 
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    if (type == 6) ImGui::SetNextWindowSizeConstraints(ImVec2(0, 0),     ImVec2(FLT_MAX, FLT_MAX), CustomConstraints::Step, (void*)100);
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::ColorEdit3("Color", &col.x);
    {
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            draw_list->AddLine(ImVec2(x, y), ImVec2(x+sz, y   ), col32, curr_thickness); x += sz+spacing;   
            draw_list->AddLine(ImVec2(x, y), ImVec2(x,    y+sz), col32, curr_thickness); x += spacing;      
            draw_list->AddLine(ImVec2(x, y), ImVec2(x+sz, y+sz), col32, curr_thickness); x += sz+spacing;   
        ...
        draw_list->AddRectFilled(ImVec2(x, y), ImVec2(x+sz, y+thickness), col32); x += sz+spacing;          
        draw_list->AddRectFilled(ImVec2(x, y), ImVec2(x+thickness, y+sz), col32); x += spacing+spacing;     
        draw_list->AddRectFilled(ImVec2(x, y), ImVec2(x+1, y+1), col32);          x += sz;                  
