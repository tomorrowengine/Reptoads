...
StructuredBuffer::StructuredBuffer( const std::wstring& name )
    ...
    , m_ElementSize(0)
{
    m_SRV = DX12Application::Get().AllocateDescriptors( D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );
    m_UAV = DX12Application::Get().AllocateDescriptors( D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );
}
