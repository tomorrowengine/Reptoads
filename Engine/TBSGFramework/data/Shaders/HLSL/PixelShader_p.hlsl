...
{
	const float _Cutoff = 0.4;
	const float _MipScale = 0.25;


    float4 texColor = DiffuseTexture.Sample( LinearRepeatSampler, IN.TexCoord );

	//texColor.a *= 1 + max(0, CalcMipLevel(i.uv * _MainTex_TexelSize.zw)) * _MipScale;

	texColor.a = (texColor.a - _Cutoff) / max(fwidth(texColor.a), 0.0001) + 0.5;
	...
	float4 pixelColor = (emissive + ambient + diffuse + specular) * texColor;
